...
void getAdjancyList(int adjMatrix[MAX][MAX], struct Node* input) {
    int adjListIndex;
    for (int i = 0; i < MAX; i++) {
        adjListIndex = 0;
        for (int j = 0; j < MAX; j++) {
        ...
        for (int j = adjListIndex; j < MAX; j++) {
             adjMatrix[i][j] = -1;
        }
    }
}
...
void getCluster(struct Node output[MAX], struct Node input[MAX], int adjList[MAX][MAX], int marked[MAX]) {
    ...
    while (ptr_cluster < MAX) {
        if (marked[ptr_cluster] == -1) {
            ...
            while (ptr_cluster_queue < MAX && output[ptr_cluster_queue].id != -1) {
...
void initialize(int keys[MAX][3], struct Node input[MAX], struct Node output[MAX], int adjMatrix[MAX][MAX], int marked[MAX]) {
...
int main() {
    ...
    getAdjancyMatrix(adj, input);
    getAdjancyList(adj, input);
    getCluster(output, input, adj, marked);
